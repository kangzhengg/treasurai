...
def analyze_financials(erp_path_str: str):
    ...
    try:
        # Reduced max_news to 3 to heavily reduce payload size and prevent 504 Timeouts
        payload = refresh_once(erp_path=erp_path, max_news=3)
    except Exception as e:
        # Fallback if data ingestion fails
    ...
    try:
        ...
        # Check if GLM returned an error or is a mock
        if "error" in decision and decision.get("error"):
             # Use fallback if GLM has a critical error
    ...
    # If GLM suggests FX conversion, run detailed simulation
    ...
    # 5. Final Orchestrated Result
    result = {
        "status": "success",
        "orchestration_flow": "Data -> GLM -> Simulation",
        "timestamp": payload["market_context"]["fetched_at_utc"],
        "glm_decision": decision,
        "simulations": simulations,
        "cash_flow_projection": cash_flow_projection,
        "metadata": {
            "cash_balance_count": len(payload["company_data"]["cash_balances"].get("accounts", [])),
            "news_items_count": len(payload["news_data"]),
...
def simulate_scenario(scenario_name: str, erp_path_str: str = "dualIntelligenceLayer/erp_data.json"):
    ...
    return {
        ...
        "timestamp": payload["market_context"].get("fetched_at_utc", ""),
        "glm_decision": decision,
        "metadata": {
